...
export class RateComponent implements OnInit {
  ...
  ratesLength!: number;
  columns: string[] = ['index', 'name', 'tour', 'rating', 'comment', 'rateDate', 'delete'];
  ...
  search(event: any) {
    ...
    this.rateService.getAll().subscribe(data => {
      this.rates = data as Rate[];
      this.rates = this.rates.filter(r => r.user.name.toLowerCase().includes(fValue.toLowerCase()) || r.tour.name.toLowerCase().includes(fValue.toLowerCase()) || r.comment.toLowerCase().includes(fValue.toLowerCase()));
      this.listData = new MatTableDataSource(this.rates);
      this.listData.sort = this.sort;
      this.listData.paginator = this.paginator;
      this.ratesLength = this.rates.length;
